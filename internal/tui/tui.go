package tui

import (
	"bufio"
	"fmt"
	"os"
	"strconv"
	"strings"

	"github.com/ixianjia/ceph-calc/internal/calc"
)

type wizardInput struct {
	usable       string
	protection   calc.ProtectionMode
	replFactor   int
	ecK, ecM     int
	diskType     calc.DiskType
	diskSize     uint64
	disksPerNode int
	safetyRatio  float64
	nodeCount    int
	services     calc.AdditionalService
	release      calc.CephRelease
}

func stepRow(num int, total int, title string) string {
	return fmt.Sprintf("  %s  ·  %s",
		DimStyle.Render(fmt.Sprintf("Step %d/%d", num, total)),
		BoldStyle.Render(title))
}

func prompt(scanner *bufio.Scanner, label, defaultValue string) string {
	val := defaultValue
	parts := strings.SplitN(label, "%s", 2)
	if len(parts) == 2 {
		label = parts[0] + val + parts[1]
	}
	fmt.Printf("    %s  (default: %s) > ", label, DimStyle.Render(val))
	if scanner.Scan() {
		t := strings.TrimSpace(scanner.Text())
		if t != "" {
			val = t
		}
	}
	return val
}

func Run() {
	scanner := bufio.NewScanner(os.Stdin)
	fmt.Println()
	fmt.Printf("  %s  ·  %s\n\n", AccentStyle.Render("ceph-calc"), DimStyle.Render("Ceph Capacity Planner"))

	in := wizardInput{
		disksPerNode: 12,
		safetyRatio:  0.80,
		diskSize:     16e12,
		diskType:     calc.DiskHDD,
		protection:   calc.ProtectionRepl,
		replFactor:   3,
	}

	// Step 1: Usable capacity
	fmt.Println(stepRow(1, 9, "Target Usable Capacity"))
	in.usable = prompt(scanner, "Capacity", "100TB")
	if _, err := calc.ParseCapacity(in.usable); err != nil {
		fmt.Printf("\n  %s\n", RedStyle.Render("error: "+err.Error()))
		return
	}
	fmt.Println()

	// Step 2: Protection strategy
	fmt.Println(stepRow(2, 9, "Protection"))
	fmt.Printf("    %d  %s\n", 1, "Replication x3")
	fmt.Printf("    %d  %s\n", 2, "Erasure Coding")
	protStr := prompt(scanner, "Choose", "1")
	switch strings.TrimSpace(protStr) {
	case "2":
		in.protection = calc.ProtectionEC
		fmt.Println()
		fmt.Println(stepRow(2, 9, DimStyle.Render("EC Parameters")))
		ek := prompt(scanner, "Data chunks K", "4")
		em := prompt(scanner, "Parity chunks M", "2")
		in.ecK, _ = strconv.Atoi(ek)
		in.ecM, _ = strconv.Atoi(em)
		if in.ecK < 1 {
			in.ecK = 4
		}
		if in.ecM < 1 {
			in.ecM = 2
		}
	default:
		in.replFactor = 3
	}
	fmt.Println()

	// Step 3: Disk type
	fmt.Println(stepRow(3, 9, "Disk Type"))
	fmt.Printf("    %d  %s\n", 1, "HDD   bulk storage")
	fmt.Printf("    %d  %s\n", 2, "SSD   balanced")
	fmt.Printf("    %d  %s\n", 3, "NVMe  high performance")
	dtStr := prompt(scanner, "Choose", "1")
	switch strings.TrimSpace(dtStr) {
	case "2":
		in.diskType = calc.DiskSSD
	case "3":
		in.diskType = calc.DiskNVME
	}
	fmt.Println()

	// Step 4: Disk size
	fmt.Println(stepRow(4, 9, "Disk Size"))
	sizes := []struct {
		label string
		bytes uint64
	}{
		{"1TB", 1e12}, {"2TB", 2e12}, {"4TB", 4e12}, {"8TB", 8e12},
		{"12TB", 12e12}, {"16TB", 16e12}, {"18TB", 18e12},
		{"20TB", 20e12}, {"22TB", 22e12}, {"24TB", 24e12},
	}
	fmt.Printf("    ")
	for i, s := range sizes {
		if i > 0 && i%5 == 0 {
			fmt.Printf("\n    ")
		}
		fmt.Printf("%d %s  ", i+1, DimStyle.Render(s.label))
	}
	fmt.Println()
	defIdx := 5
	if in.diskType == calc.DiskNVME {
		defIdx = 2
	}
	szStr := prompt(scanner, "Choose", sizes[defIdx].label)
	szStr = strings.TrimSpace(szStr)
	szIdx, err := strconv.Atoi(szStr)
	if err != nil || szIdx < 1 || szIdx > len(sizes) {
		for i, s := range sizes {
			if strings.EqualFold(szStr, s.label) {
				szIdx = i + 1
				break
			}
		}
		if szIdx < 1 {
			szIdx = defIdx + 1
		}
	}
	in.diskSize = sizes[szIdx-1].bytes
	fmt.Println()

	// Step 5: Disks per node
	fmt.Println(stepRow(5, 9, "Disks per Node"))
	dpn := prompt(scanner, "Max disks", "12")
	in.disksPerNode, _ = strconv.Atoi(dpn)
	if in.disksPerNode < 1 || in.disksPerNode > 36 {
		in.disksPerNode = 12
	}
	fmt.Println()

	// Step 6: Safety ratio
	fmt.Println(stepRow(6, 9, "Safety Ratio"))
	sr := prompt(scanner, "Ratio (0.50-0.95)", "0.80")
	in.safetyRatio, _ = strconv.ParseFloat(sr, 64)
	if in.safetyRatio < 0.50 || in.safetyRatio > 0.95 {
		in.safetyRatio = 0.80
	}
	fmt.Println()

	// Step 7: Additional services
	fmt.Println(stepRow(7, 9, "Additional Services"))
	fmt.Printf("    %d  %s\n", 1, "OSD only")
	fmt.Printf("    %d  %s\n", 2, "OSD + CephFS (MDS)")
	fmt.Printf("    %d  %s\n", 3, "OSD + RGW")
	fmt.Printf("    %d  %s\n", 4, "OSD + CephFS + RGW")
	svcStr := prompt(scanner, "Choose", "1")
	switch strings.TrimSpace(svcStr) {
	case "2":
		in.services = calc.ServiceCephFS
	case "3":
		in.services = calc.ServiceRGW
	case "4":
		in.services = calc.ServiceBoth
	}
	fmt.Println()

	// Step 8: Ceph release
	fmt.Println(stepRow(8, 9, "Ceph Release"))
	fmt.Printf("    %d  %s\n", 1, "Pacific (v16)")
	fmt.Printf("    %d  %s\n", 2, "Quincy (v17)")
	fmt.Printf("    %d  %s\n", 3, "Reef (v18)")
	fmt.Printf("    %d  %s\n", 4, "Squid (v19)")
	rlsStr := prompt(scanner, "Choose", "1")
	switch strings.TrimSpace(rlsStr) {
	case "1":
		in.release = calc.ReleasePacific
	case "2":
		in.release = calc.ReleaseQuincy
	case "3":
		in.release = calc.ReleaseReef
	case "4":
		in.release = calc.ReleaseSquid
	}
	fmt.Println()

	// Step 9: Node count
	fmt.Println(stepRow(9, 9, "Node Count"))
	fmt.Printf("    %s\n", DimStyle.Render("0=auto, or specify exact count"))
	nc := prompt(scanner, "Nodes", "0")
	in.nodeCount, _ = strconv.Atoi(nc)
	if in.nodeCount < 0 {
		in.nodeCount = 0
	}
	fmt.Println()

	// Calculate
	fmt.Printf("  %s\n\n", DimStyle.Render("calculating..."))

	usableBytes, _ := calc.ParseCapacity(in.usable)
	calcResult := calc.Calculate(calc.Input{
		TargetUsable:       usableBytes,
		Protection:         in.protection,
		ReplFactor:         in.replFactor,
		ECK:                in.ecK,
		ECM:                in.ecM,
		DiskSize:           in.diskSize,
		DiskType:           in.diskType,
		DisksPerNode:       in.disksPerNode,
		SafetyRatio:        in.safetyRatio,
		NodeCount:          in.nodeCount,
		AdditionalServices: in.services,
		CephRelease:        in.release,
	})

	fmt.Println(renderResult(in.usable, calcResult, sizes[szIdx-1].label))

	fmt.Println()
	fmt.Printf("  %s\n", DimStyle.Render("ctrl+c to exit"))
	fmt.Scanln()
}

func renderResult(usable string, r calc.Result, diskLabel string) string {
	prot := ProtectionLabel(r.Input.Protection, r.Input.ReplFactor, r.Input.ECK, r.Input.ECM)

	var b strings.Builder

	b.WriteString("\n  " + Separator + "\n\n")

	// Target
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("Target")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Capacity", usable))
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Protection", prot))
	b.WriteString(fmt.Sprintf("    %-14s %s %s\n", "Disk", DiskTypeLabel(r.Input.DiskType), calc.FormatBytes(r.Input.DiskSize)))
	b.WriteString(fmt.Sprintf("    %-14s %.0f%%\n", "Safety", r.Input.SafetyRatio*100))
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Release", ReleaseLabel(r.Input.CephRelease)))

	b.WriteString("\n  " + Separator + "\n\n")

	// Hardware Plan
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("Hardware Plan")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	nodeLabel := fmt.Sprintf("%d  (auto)", r.Nodes)
	if !r.IsAutoNodes {
		nodeLabel = fmt.Sprintf("%d  (user-specified)", r.Nodes)
	}
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Nodes", AccentStyle.Render(nodeLabel)))
	if r.NodeWarning != "" {
		b.WriteString(fmt.Sprintf("    %s\n", RedStyle.Render("  ⚠  "+r.NodeWarning)))
	}
	b.WriteString(fmt.Sprintf("    %-14s %d\n", "OSDs per node", r.OSDsPerNode))
	b.WriteString(fmt.Sprintf("    %-14s %d\n", "Total OSDs", r.TotalOSDs))
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Raw", calc.FormatBytes(r.RawCapacity)))
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Usable", calc.FormatBytes(r.UsableCapacity)))

	eff := fmt.Sprintf("%.1f%%", r.Efficiency)
	switch {
	case r.Efficiency >= 40:
		b.WriteString(fmt.Sprintf("    %-14s %s\n", "Efficiency", GreenStyle.Render(eff)))
	case r.Efficiency >= 25:
		b.WriteString(fmt.Sprintf("    %-14s %s\n", "Efficiency", AccentStyle.Render(eff)))
	default:
		b.WriteString(fmt.Sprintf("    %-14s %s\n", "Efficiency", RedStyle.Render(eff)))
	}

	b.WriteString("\n  " + Separator + "\n\n")

	// Node Spec
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("Node Spec (per node)")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	b.WriteString(fmt.Sprintf("    %-14s %d cores (%s)\n", "CPU", r.Hardware.CPUCoresPerNode, r.Hardware.CPUDesc))
	b.WriteString(fmt.Sprintf("    %-14s %.0f GB (%s)\n", "RAM", r.Hardware.RAMGBPerNode, r.Hardware.RAMDesc))
	b.WriteString(fmt.Sprintf("    %-14s %d x %s\n", "OSDs", r.OSDsPerNode, calc.FormatBytes(r.Input.DiskSize)))
	b.WriteString(fmt.Sprintf("    %-14s %s public + %s cluster\n", "Network", r.Network.PublicSpeed, r.Network.ClusterSpeed))
	b.WriteString(fmt.Sprintf("    %-14s %s\n", "Services", r.Hardware.ServicesDesc))

	b.WriteString("\n  " + Separator + "\n\n")

	// DB/WAL
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("DB/WAL")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	b.WriteString(fmt.Sprintf("  %s\n", r.Hardware.DBWALDesc))

	b.WriteString("\n  " + Separator + "\n\n")

	// Network Plan
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("Network Plan")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	b.WriteString(fmt.Sprintf("    %-14s %s  %s\n", "Public", r.Network.PublicSpeed, DimStyle.Render(r.Network.PublicNetwork)))
	b.WriteString(fmt.Sprintf("    %-14s %s  %s\n", "Cluster", r.Network.ClusterSpeed, DimStyle.Render(r.Network.ClusterNetwork)))

	b.WriteString("\n  " + Separator + "\n\n")

	// PG Suggestion
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("PG Suggestion")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	for _, pg := range r.PGSuggestions {
		b.WriteString(fmt.Sprintf("    %-22s %s\n", pg.PoolName, fmt.Sprintf("%d PGs", pg.PGCount)))
	}
	if r.Hardware.PGConfigDesc != "" {
		for _, line := range strings.Split(r.Hardware.PGConfigDesc, "\n") {
			b.WriteString("  " + DimStyle.Render(line) + "\n")
		}
	}

	b.WriteString("\n  " + Separator + "\n\n")

	// CLI Command
	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("CLI")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	cmd := "ceph-calc"
	cmd += " --usable " + usable
	if r.Input.Protection == calc.ProtectionRepl {
		cmd += fmt.Sprintf(" --repl %d", r.Input.ReplFactor)
	} else {
		cmd += fmt.Sprintf(" --ec-k %d --ec-m %d", r.Input.ECK, r.Input.ECM)
	}
	cmd += fmt.Sprintf(" --disk %s --disk-type %s", strings.TrimSpace(diskLabel), r.Input.DiskType)
	cmd += fmt.Sprintf(" --disks-per-node %d", r.Input.DisksPerNode)
	b.WriteString(fmt.Sprintf("    %s\n", cmd))

	b.WriteString("\n  " + Separator + "\n\n")

	b.WriteString(fmt.Sprintf("  %s\n", BoldStyle.Render("Cephadm Spec")))
	b.WriteString(fmt.Sprintf("  %s\n", DimStyle.Render("─────────────────────────────────")))
	b.WriteString(fmt.Sprintf("    %s --gen-spec > cluster.yaml\n", cmd))

	b.WriteString("\n  " + Separator + "\n")

	return b.String()
}
